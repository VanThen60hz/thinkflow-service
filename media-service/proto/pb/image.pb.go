// Code generated by protoc-gen-go. DO NOT EDIT.
// versions:
// 	protoc-gen-go v1.36.5
// 	protoc        (unknown)
// source: proto/image.proto

package pb

import (
	protoreflect "google.golang.org/protobuf/reflect/protoreflect"
	protoimpl "google.golang.org/protobuf/runtime/protoimpl"
	reflect "reflect"
	sync "sync"
	unsafe "unsafe"
)

const (
	// Verify that this generated code is sufficiently up-to-date.
	_ = protoimpl.EnforceVersion(20 - protoimpl.MinVersion)
	// Verify that runtime/protoimpl is sufficiently up-to-date.
	_ = protoimpl.EnforceVersion(protoimpl.MaxVersion - 20)
)

type GetImageByIdReq struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	Id            int32                  `protobuf:"varint,1,opt,name=id,proto3" json:"id,omitempty"` // Using int32 for compatibility with proto conventions
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *GetImageByIdReq) Reset() {
	*x = GetImageByIdReq{}
	mi := &file_proto_image_proto_msgTypes[0]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *GetImageByIdReq) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*GetImageByIdReq) ProtoMessage() {}

func (x *GetImageByIdReq) ProtoReflect() protoreflect.Message {
	mi := &file_proto_image_proto_msgTypes[0]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use GetImageByIdReq.ProtoReflect.Descriptor instead.
func (*GetImageByIdReq) Descriptor() ([]byte, []int) {
	return file_proto_image_proto_rawDescGZIP(), []int{0}
}

func (x *GetImageByIdReq) GetId() int32 {
	if x != nil {
		return x.Id
	}
	return 0
}

type GetImagesByIdsReq struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	Ids           []int32                `protobuf:"varint,1,rep,packed,name=ids,proto3" json:"ids,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *GetImagesByIdsReq) Reset() {
	*x = GetImagesByIdsReq{}
	mi := &file_proto_image_proto_msgTypes[1]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *GetImagesByIdsReq) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*GetImagesByIdsReq) ProtoMessage() {}

func (x *GetImagesByIdsReq) ProtoReflect() protoreflect.Message {
	mi := &file_proto_image_proto_msgTypes[1]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use GetImagesByIdsReq.ProtoReflect.Descriptor instead.
func (*GetImagesByIdsReq) Descriptor() ([]byte, []int) {
	return file_proto_image_proto_rawDescGZIP(), []int{1}
}

func (x *GetImagesByIdsReq) GetIds() []int32 {
	if x != nil {
		return x.Ids
	}
	return nil
}

type NewImageIdResp struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	Id            int32                  `protobuf:"varint,1,opt,name=id,proto3" json:"id,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *NewImageIdResp) Reset() {
	*x = NewImageIdResp{}
	mi := &file_proto_image_proto_msgTypes[2]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *NewImageIdResp) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*NewImageIdResp) ProtoMessage() {}

func (x *NewImageIdResp) ProtoReflect() protoreflect.Message {
	mi := &file_proto_image_proto_msgTypes[2]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use NewImageIdResp.ProtoReflect.Descriptor instead.
func (*NewImageIdResp) Descriptor() ([]byte, []int) {
	return file_proto_image_proto_rawDescGZIP(), []int{2}
}

func (x *NewImageIdResp) GetId() int32 {
	if x != nil {
		return x.Id
	}
	return 0
}

type PublicImageInfo struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	Id            int64                  `protobuf:"varint,1,opt,name=id,proto3" json:"id,omitempty"`              // Matches core.Image.Id (int64)
	Url           string                 `protobuf:"bytes,2,opt,name=url,proto3" json:"url,omitempty"`             // Matches core.Image.Url (string)
	Width         int64                  `protobuf:"varint,3,opt,name=width,proto3" json:"width,omitempty"`        // Matches core.Image.Width (int64)
	Height        int64                  `protobuf:"varint,4,opt,name=height,proto3" json:"height,omitempty"`      // Matches core.Image.Height (int64)
	Extension     string                 `protobuf:"bytes,5,opt,name=extension,proto3" json:"extension,omitempty"` // Matches core.Image.Extension (string)
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *PublicImageInfo) Reset() {
	*x = PublicImageInfo{}
	mi := &file_proto_image_proto_msgTypes[3]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *PublicImageInfo) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*PublicImageInfo) ProtoMessage() {}

func (x *PublicImageInfo) ProtoReflect() protoreflect.Message {
	mi := &file_proto_image_proto_msgTypes[3]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use PublicImageInfo.ProtoReflect.Descriptor instead.
func (*PublicImageInfo) Descriptor() ([]byte, []int) {
	return file_proto_image_proto_rawDescGZIP(), []int{3}
}

func (x *PublicImageInfo) GetId() int64 {
	if x != nil {
		return x.Id
	}
	return 0
}

func (x *PublicImageInfo) GetUrl() string {
	if x != nil {
		return x.Url
	}
	return ""
}

func (x *PublicImageInfo) GetWidth() int64 {
	if x != nil {
		return x.Width
	}
	return 0
}

func (x *PublicImageInfo) GetHeight() int64 {
	if x != nil {
		return x.Height
	}
	return 0
}

func (x *PublicImageInfo) GetExtension() string {
	if x != nil {
		return x.Extension
	}
	return ""
}

type PublicImageInfoResp struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	Image         *PublicImageInfo       `protobuf:"bytes,1,opt,name=image,proto3" json:"image,omitempty"` // Singular response
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *PublicImageInfoResp) Reset() {
	*x = PublicImageInfoResp{}
	mi := &file_proto_image_proto_msgTypes[4]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *PublicImageInfoResp) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*PublicImageInfoResp) ProtoMessage() {}

func (x *PublicImageInfoResp) ProtoReflect() protoreflect.Message {
	mi := &file_proto_image_proto_msgTypes[4]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use PublicImageInfoResp.ProtoReflect.Descriptor instead.
func (*PublicImageInfoResp) Descriptor() ([]byte, []int) {
	return file_proto_image_proto_rawDescGZIP(), []int{4}
}

func (x *PublicImageInfoResp) GetImage() *PublicImageInfo {
	if x != nil {
		return x.Image
	}
	return nil
}

type PublicImagesInfoResp struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	Images        []*PublicImageInfo     `protobuf:"bytes,1,rep,name=images,proto3" json:"images,omitempty"` // Repeated for multiple images
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *PublicImagesInfoResp) Reset() {
	*x = PublicImagesInfoResp{}
	mi := &file_proto_image_proto_msgTypes[5]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *PublicImagesInfoResp) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*PublicImagesInfoResp) ProtoMessage() {}

func (x *PublicImagesInfoResp) ProtoReflect() protoreflect.Message {
	mi := &file_proto_image_proto_msgTypes[5]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use PublicImagesInfoResp.ProtoReflect.Descriptor instead.
func (*PublicImagesInfoResp) Descriptor() ([]byte, []int) {
	return file_proto_image_proto_rawDescGZIP(), []int{5}
}

func (x *PublicImagesInfoResp) GetImages() []*PublicImageInfo {
	if x != nil {
		return x.Images
	}
	return nil
}

var File_proto_image_proto protoreflect.FileDescriptor

var file_proto_image_proto_rawDesc = string([]byte{
	,
})

var (
	file_proto_image_proto_rawDescOnce sync.Once
	file_proto_image_proto_rawDescData []byte
)

func file_proto_image_proto_rawDescGZIP() []byte {
	file_proto_image_proto_rawDescOnce.Do(func() {
		file_proto_image_proto_rawDescData = protoimpl.X.CompressGZIP(unsafe.Slice(unsafe.StringData(file_proto_image_proto_rawDesc), len(file_proto_image_proto_rawDesc)))
	})
	return file_proto_image_proto_rawDescData
}

var file_proto_image_proto_msgTypes = make([]protoimpl.MessageInfo, 6)
var file_proto_image_proto_goTypes = []any{
	(*GetImageByIdReq)(nil),      // 0: pb.GetImageByIdReq
	(*GetImagesByIdsReq)(nil),    // 1: pb.GetImagesByIdsReq
	(*NewImageIdResp)(nil),       // 2: pb.NewImageIdResp
	(*PublicImageInfo)(nil),      // 3: pb.PublicImageInfo
	(*PublicImageInfoResp)(nil),  // 4: pb.PublicImageInfoResp
	(*PublicImagesInfoResp)(nil), // 5: pb.PublicImagesInfoResp
}
var file_proto_image_proto_depIdxs = []int32{
	3, // 0: pb.PublicImageInfoResp.image:type_name -> pb.PublicImageInfo
	3, // 1: pb.PublicImagesInfoResp.images:type_name -> pb.PublicImageInfo
	0, // 2: pb.ImageService.GetImageById:input_type -> pb.GetImageByIdReq
	1, // 3: pb.ImageService.GetImagesByIds:input_type -> pb.GetImagesByIdsReq
	4, // 4: pb.ImageService.GetImageById:output_type -> pb.PublicImageInfoResp
	5, // 5: pb.ImageService.GetImagesByIds:output_type -> pb.PublicImagesInfoResp
	4, // [4:6] is the sub-list for method output_type
	2, // [2:4] is the sub-list for method input_type
	2, // [2:2] is the sub-list for extension type_name
	2, // [2:2] is the sub-list for extension extendee
	0, // [0:2] is the sub-list for field type_name
}

func init() { file_proto_image_proto_init() }
func file_proto_image_proto_init() {
	if File_proto_image_proto != nil {
		return
	}
	type x struct{}
	out := protoimpl.TypeBuilder{
		File: protoimpl.DescBuilder{
			GoPackagePath: reflect.TypeOf(x{}).PkgPath(),
			RawDescriptor: unsafe.Slice(unsafe.StringData(file_proto_image_proto_rawDesc), len(file_proto_image_proto_rawDesc)),
			NumEnums:      0,
			NumMessages:   6,
			NumExtensions: 0,
			NumServices:   1,
		},
		GoTypes:           file_proto_image_proto_goTypes,
		DependencyIndexes: file_proto_image_proto_depIdxs,
		MessageInfos:      file_proto_image_proto_msgTypes,
	}.Build()
	File_proto_image_proto = out.File
	file_proto_image_proto_goTypes = nil
	file_proto_image_proto_depIdxs = nil
}
