...
func (c *rpcClient) GetUsersByIds(ctx context.Context, ids []int) ([]core.SimpleUser, error) {
	...
	for i := range users {
		...
		users[i] = core.NewSimpleUser(int(respUser.Id), respUser.Email, respUser.FirstName, respUser.LastName, int(respUser.AvatarId))
	}
...
func (c *rpcClient) GetUserById(ctx context.Context, id int) (*core.SimpleUser, error) {
	...
	user := core.NewSimpleUser(int(resp.User.Id), resp.User.Email, resp.User.FirstName, resp.User.LastName, int(resp.User.AvatarId))
