...
func (biz *business) DeleteTranscript(ctx context.Context, id int) error {
	...
	if err := biz.transcriptRepo.DeleteTranscript(ctx, id); err != nil {
		return core.ErrInternalServerError.
			WithError(entity.ErrCannotDeleteTranscript.Error()).
			WithDebug(err.Error())
	}
